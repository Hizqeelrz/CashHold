defmodule CashHoldWeb.Api.BankTransactionController do
  ...
  defp csv_content do
    ...
    #   query = if not is_nil(params["start_date"]), do: (from t in query, where: t.inserted_at > ^start_date), else: query
    #   query = if not is_nil(params["end_date"]), do: (from t in query, where: t.inserted_at < ^end_date), else: query
  
    #   query = if params["name"] == "balance", do: (from t in query, where: t.balance > ^min and t.balance < ^max), else: query
    #   query = if params["name"] == "deposit", do: (from t in query, where: t.deposit > ^min and t.deposit < ^max), else: query
    #   query = if params["name"] == "withdraw", do: (from t in query, where: t.withdraw > ^min and t.withdraw < ^max), else: query
